				...
				<div class="control-group">
            		...
            		<div class="controls controls-row">
        				<select name="Propinsi" id="Propinsi" class="span5"  >
        					<option value="0"> - Pilih Propinsi yang akan di Tampilkan </option>
...
foreach($arr_pemeriksaan as $value):
    ...
    ?>
								<tr id='id_revco_<?php echo $value['id_revco']; ?>'>
									<td>
										<?php echo $no ?>
                                        <input type="hidden" name="id_revco" value="<?php echo $value['P_ID']; ?>" />
                                        <input type="hidden" name="Lab" value="<?php echo $value['no_stiker']; ?>" />
                                        <input type="hidden" name="jenisrev" value="<?php echo $value['GAMBAR_UPLOAD']; ?>" /> </td>
									<td class='table-icon'>
    ...
    if (!($level == 1 || $level == 2))
        echo "
											<a href=\"#\" onclick=\"document.dialog_click('{$value['P_ID']}')\" id=\"dialog-link\" data-toggle=\"modal\" class='button button-basic button-icon' rel=\"tooltip\" title=\"View\" id_revco=\"{$value['P_ID']}\"><i class=\"icon-eye-open\"></i></a>
        ";
    ?>
										<?php
    if (!($level == 3))
        echo"
        <a href=\"#\" onclick=\"document.dialog_click('{$value['P_ID']}');document.frmrevco.id_revco.value=$value[P_ID]\" id=\"dialog-link\" data-toggle=\"modal\" class='button button-basic button-icon' rel=\"tooltip\" title=\"Edit\" id_revco=\"{$value['P_ID']}\"><i class=\"icon-exclamation-sign\"></i></a>
        <a href=\"#\" onclick=\"delete_link('{$value['P_ID']}')\" class='button button-basic button-icon' rel=\"tooltip\" title=\"Delete\"><i class=\"icon-trash\"></i></a>
										";
    ?>
										</div>
									</center>
