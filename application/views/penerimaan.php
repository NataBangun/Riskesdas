<script type="text/javascript">
        ...
        function addpenerimaan_submit(){
			...
			else if(document.penerimaan.darah.value=='0')
			{alert('Diambil Darah Harus di Isi.');}
        ...
        function kabupaten(){
            $.ajax({
				url:'<?php echo $application_path;?>/penerimaan/kabupaten/'+$ID('Prov').value,
				complete: function(res, status){
					//var kotak = res.responseText;
        ...
        function kecamatan(){
			$.ajax({
				url:'<?php echo $application_path;?>/penerimaan/kecamatan/'+$ID('Prov').value+'/'+$ID('Kota').value,
				complete: function(res, status){
					//var kotak = res.responseText;
        ...
        function kelurahan(){
			$.ajax({
				url:'<?php echo $application_path;?>/penerimaan/kelurahan/'+$ID('Prov').value+'/'+$ID('Kota').value+'/'+$ID('Kec').value,
				complete: function(res, status){
					//var kotak = res.responseText;
	...
	function reload(value) {
		jQuery.ajax({
			...
			complete: function(res, status) {
				...
				var vdarah = document.penerimaan.darah;
	...
	function nobarcode(event) {
		...
		if (event.keyCode == 13) {
			...
			if(document.penerimaan.NoBarcode.value.length==13){
				...
				combobox_select(document.penerimaan.Prov,vNoBarcode.substring(3,5));
				kabupaten();
				...
				combobox_select(document.penerimaan.JenisSpesimen,vNoBarcode.substring(12,13));
        ...
        function kosong(){
            $ID('NamaART').value = '';
            $ID('umurART').value = '';
            combobox_select($ID('JK'), 0);
            combobox_select($ID('darah'), 0);
            $ID('Alamat').value = '';
            combobox_select($ID('Prov'),0);
            combobox_select($ID('Kota'),0);
            combobox_select($ID('Kec'),0);
            combobox_select($ID('Desa'),0);
            combobox_select($ID('KondisiSpesimen'),0);
            $ID('SuhuDtg').value = '';
            $ID('JSpesimen').value = '';
            $ID('VSpesimen').value = '';
            combobox_select($ID('NamaSite'),0);
            combobox_select($ID('SimpanSpesimen'),0);
            $ID('TGLKirim').value = '';
            $ID('Pengirim').value = '';
            $ID('VSpesimen').value = '';
            $ID('DiambilTanggal').value = '';
            $ID('TGLDiterima').value = '';
            $ID('AWB').value = '';
            $ID('Ket').value = '';
        }
				...
				<div class="box-body">
					<form method="POST" id="frm1" name="penerimaan" class='form-horizontal'>
						...
						<div class="control-group">
							...
							<div class="controls controls-row">
								<input type="text" name="NoBarcode" id="NoBarcode" class="span3" onkeyup="kosong();nobarcode(event);" /><label id="barcodevalid" style="color:red;"><b>&nbsp; *sudah ada</b></label>
						...
						<div class="control-group">
							...
							<div class="controls controls-row">
								...
								<input style="margin-left: 2.5641%;" type="text" name="Visit" id="Visit" placeholder="" class="span1" disabled="true"/>
						...
						<div class="control-group">
							...
							<div class="controls controls-row">
								...
								<label style="margin-left: 7.5%;width:15%;" for="VSpesimen" class="control-label">Volume Spesimen (&#181;m)</label>
						...
						<label for="NamaART" class="control-label">Nama</label>
							...
							<div class="controls controls-row">
								<label style="margin-left:3%" for="NamaSite" class="control-label">Nama Site</label>
						...
						<label for="umurART" class="control-label">Umur</label>
							<div class="controls controls-row">
								...
								<label style="margin-left:22.55%" for="SimpanSpesimen" class="control-label">Spesimen disimpan di</label>
                        ...
                        <div class="control-group">
							...
							<div class="controls controls-row">
								...
								<label style="margin-left:17%" for="TGLKirim" class="control-label">TGL Kirim</label>
						...
						<div class="control-group">
							<label for="darah" class="control-label">Diambil Spesimen</label>
							<div class="controls controls-row">
								<select name="darah" id="darah" class='span3'>
									<option value="0"> - Diambil Spesimen - </option>
						...
						<div class="control-group">
							...
							<div class="controls controls-row">
								...
								<div class="span4">
									<select name="Prov" id="Prov"  onchange="kabupaten(this.value);document.frm1.Prov.value=this.value;" readonly="true">
