...
<table width="1000" height="100%" border="0" align="center" cellpadding="4">
  ...
  <tr>
    <td colspan="8"><table width="100%" border="0" cellpadding="3" cellspacing="0" class="tabel">
      ...
      <?php
      $i=$_REQUEST['ambilJumlah'];
      echo $_REQUEST['no_barcode1'];
      for($a=1;$a<=$i;$a++){
          echo $_REQUEST['no_barcode'.$a];
        if(isset($_REQUEST['no_barcode'.$a])&&$_REQUEST['no_barcode'.$a]!=''){
              echo '<tr>
                <td class="tabel"><div >'.$a.'</div></td>
                <td class="tabel"><div >'.$_REQUEST['no_barcode'.$a].'</div></td>
                <td class="tabel"><div >'.$_REQUEST['dt_tglterima'.$a].'</div></td>
                <td class="tabel"><div >'.$_REQUEST['dt_kondisi'.$a].'</div></td>
                <td class="tabel"><div >'.$_REQUEST['dt_spesimen'.$a].'</div></td>
