...
class view_export extends main {
	...
	{	
		...
		$query = $this->db->query("SELECT 
									...
									WHERE t1.propinsi_id=? AND t1.kode_lab=? AND t1.kode_penelitian=? ORDER BY t1.no_stiker ASC", array ($prop,$lab,$penelitian,$tgl1,$tgl2));
		...
		$query = $this->db->query("SELECT 
									...
									left join formhasil t3 on t2.noStiker=t3.no_stiker 
									left join formmalarianew t4 on t3.no_stiker=t4.no_stiker 
									WHERE t1.propinsi_id is null and t2.propinsi_id=? AND t1.kode_lab=? AND t1.kode_penelitian=? ORDER BY t1.no_stiker ASC", array ($prop,$lab,$penelitian,$tgl1,$tgl2));
