...
class pengambilan extends main{
                            ...
                            echo'</td>
                                <td>
                                    ...
                                    if(!in_array($value['no_barcode'],$dataambil)){
                                        echo (($value['status']==0)?'<a id="ambil'.$i.'" href="#" class="btn" onclick="ambil('.$i.')">Ambil</a>':'');
                                    }
                                    '
                                </td>
                            </tr>
	...
	public function buat_faktur(){
        $dataMax = mysql_fetch_assoc(mysql_query("SELECT max(no_formpengambilan) AS last FROM formpengambilan WHERE no_formpengambilan LIKE '$today%'"));
		$datalast = $data['last'];
		$noakhir = $datalast; 
		...
		$kode = "TESS-";
		$year = date('ymd');
		$nope = substr($noakhir, 12, 4);
		$nope = (int)$nope + 1;
		$nope = $kode . $year . substr($urut, 0, 4 - strlen($nope)) . $nope;
        //var_dump($nope);
        //print($nope);
        $data['auto'] = $nope;
    ...
    function transaksi_id($param='inv-str') {
        $dataMax = mysql_fetch_assoc(mysql_query("SELECT SUBSTR(MAX(`no_formpengambilan`),-5) AS ID  FROM formpengambilan"));
        ...
        if (isset($_POST['status_pengambilan'])) {
            if($_POST['status_pengambilan']==0){
                $this->db->query("
                    ...
                    no_formpengambilan  = '.transaksi_id().',
