...
class formhasil_malarianew extends main {
	...
	public function cek($id = ""){
		...
		$query = $this->db->query("SELECT * FROM formmalarianew WHERE no_stiker='$nostiker' AND kode_lab LIKE '%{$this->data['lab']}%' AND kode_penelitian LIKE '%{$this->data['penelitian']}%'");
		$rows = $query->result_array();
	...
	{	
		parent::index();
			...
			if (isset($_POST['status_addmalaria'])) {
				// 1. Insert Data
				// var_dump($this->db);
				$this->db->query("insert formmalarianew set kode_lab=?, kode_penelitian=?, propinsi_id=?, kabupaten_id=?, kecamatan_id=?, kelurahan_id=?, DK=?, kode_sampel=?, no_bangun_sensus=?, no_urut_sampel=?,
