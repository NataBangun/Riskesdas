...
class view_exportjenpen extends main {
	...
	{	
		...
		// header('Content-Disposition: attachment; filename=laporan_penyimpanan_'.$tgl1.'_'.$tgl2.'.xls');
		header('Content-Disposition: attachment; filename=laporan_Jenis_Penyimpanan.xls');
		...
		$JenisRevco = $this->input->post('JenisRevco');
		...
		$query = $this->db->query("SELECT * FROM formpenerimaan 
								...
								WHERE formpenerimaan.spesimen_kode='{$this->input->post('JenisRevco')}' ORDER BY formpenerimaan.no_stiker ASC");
		$rows = $query->result_array();
		// var_dump($rows);
		$this->data['view_exportjenpen'] = $rows;
		...
		$this->data['area_body'] = $this->load->view('view_exportjenpen', $this->data, true);
		$this->data['judul'] = 'Form Export Kimia Klinis';
		$this->load->view('view_exportjenpen', $this->data);
	}
}
