...
//		print_r($this->data['arr_bm01']);
	...
	{	
		parent::index();
		$this->get_arr_bm01();
                $this->load->library('dbo_table');
		$dbo = $this->dbo_table;
//                $dbo->set_table('data_bm01');
                $dbo->set_query(" SELECT * FROM bm_01 
                    RIGHT JOIN data_bm01 ON data_bm01.bm01_id = bm_01.bm_01_id
                    where NoStiker LIKE '%{$this->data['search']}%' AND SamGaram LIKE '%{$this->data['search']}%' AND kode_lab LIKE '%{$this->data['lab']}%' 
                    AND kode_penelitian LIKE '%{$this->data['penelitian']}%' ");
                 
                 $dbo->set_table_head(array(
                            'bm01_id'       =>  'ID',
                            'NamaART'       =>  'Nama',
                            'kode_sampel'   =>  'Sampel'
                         ));
                 $dbo->set_atribut_table(array('class'=>'table table-nomargin table-bordered table-pagination'));
                 $this->data['table_bm01'] = $dbo->generate_table();
...
//			} else {
				$this->data['area_body'] = $this->load->view('list_test', $this->data, true);
//			}
